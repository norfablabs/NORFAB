...
query CircuitsQuery(
    ...
    circuits: circuit_list(
        filters: {
            terminations: {site: {slug: {in_list: $sites}}}
            cid: {in_list: $cids}
        }
...
CIRCUITS_FRESHNESS_QUERY = """
query CircuitsFreshnessQuery(
    $sites: [String!]
    $cids: [String!]
    $offset: Int!
    $limit: Int!
...
class NetboxCircuitsTasks:
    ...
    def get_circuits(
        ...
        **How it works:**
        ...
        # form initial query variables based on devices' sites and cid list
        ...
        query_variables = {"sites": sites or None, "cids": cid or None}
        fetch_circuits = True

        log.info(
            f"{self.name}:get_circuits - constructed circuits query variables: '{query_variables}'"
        )
        ...
        if cache == True or cache == "force":
            ...
            last_updated = self.netbox_graphql(
                job=job,
                query=CIRCUITS_FRESHNESS_QUERY,
            ...
            for device in devices:
                for circuit in last_updated.result.get("circuits", []):
            ...
            # fetch remaining circuits by CID without applying the site filter
            query_variables = {"sites": None, "cids": cid_list or None}
            fetch_circuits = bool(cid_list)
        ...
        if fetch_circuits:
            ...
            all_circuits = query_result.result.get("circuits", [])
