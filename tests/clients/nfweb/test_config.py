...
class TestNFWebConfig:
    def test_nfweb_config_separates_runtime_and_topology_settings(self) -> None:
        config = NFWebConfig()
    ...
    def test_nfweb_config_accepts_ip_bind_hosts_and_rejects_hostnames(self) -> None:
    ...
    def test_nfweb_config_rejects_long_history(self) -> None:
        with pytest.raises(ValidationError):
            TopologyConfig(retention_minutes=181)

    def test_topology_config_rejects_removed_settings(self) -> None:
        with pytest.raises(ValidationError):
            TopologyConfig.model_validate({"sites": ["dc1"]})
        with pytest.raises(ValidationError):
            TopologyConfig.model_validate({"layers": {"inventory": True}})

    def test_nfweb_footer_config_is_strict_and_bounds_the_message(self) -> None:
