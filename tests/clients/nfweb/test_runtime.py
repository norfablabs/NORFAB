...
class TestNFWebRuntime:
    def test_wildcard_bind_uses_local_ip_in_access_url(self) -> None:
        with patch(
    ...
    def test_specific_ipv6_bind_is_formatted_for_browser_url(self) -> None:
        assert _access_url("2001:db8::10", 9005) == "http://[2001:db8::10]:9005"

    def test_serve_binds_all_interfaces_from_default_config(self) -> None:
        inventory = Mock(
        ...
        with (
            ...
            patch(
                "norfab.clients.nfweb.runtime.asyncio.Event", return_value=stop_event
            ),
            patch(
    ...
    def test_signal_handlers_are_restored_after_graceful_shutdown(self) -> None:
        loop = Mock()
        stop_event = Mock()
