...
class TestTopologyCollector:
    def test_submit_job_captures_norfab_events(self) -> None:
        class EventClient:
    ...
    def test_submit_job_wait_does_not_block_event_loop(self) -> None:
    ...
    def test_device_discovery_uses_nornir_only(self) -> None:
    ...
    def test_lldp_normalizes_reverse_device_and_interface_names(self) -> None:
    ...
    def test_collector_merges_live_layers_and_interface_health(
        self, tmp_path: Path
    ) -> None:
        client = FakeClient()
        config = TopologyConfig(devices=["r1", "r2"])
        ...
        assert (
            next(node for node in snapshot.nodes if node.id == "r2").health == "healthy"
        )
        topology_link = next(
            link
    ...
    def test_collector_reports_partial_worker_failure(self, tmp_path: Path) -> None:
        ...
        assert any(
            "device connection failed" in error.message for error in snapshot.errors
        )
        asyncio.run(collector.stop())
        history.close()

    def test_collector_keeps_partial_worker_payload(self, tmp_path: Path) -> None:
        class PartialPayloadClient(FakeClient):
            def _job_result(
                ...
                if service == "netbox" and task == "get_topology":
                    response["netbox-worker"]["errors"] = [
                        "one cable could not be read"
                    ]
                return response
    ...
    def test_empty_selection_does_not_collect(self, tmp_path: Path) -> None:
        client = FakeClient()
        history = TopologyHistoryStore(tmp_path / "nfweb.sqlite")
    ...
    def test_device_selection_collects_selected_scope(self, tmp_path: Path) -> None:
        client = FakeClient()
        history = TopologyHistoryStore(tmp_path / "nfweb.sqlite")
        collector = TopologyCollector(client, TopologyConfig(), history)
