...
class TestTopologyHistory:
    def test_history_round_trip(self, tmp_path: Path) -> None:
        ...
        assert [entry.snapshot_id for entry in store.history(["r1"])] == [
            first.snapshot_id
        ]
        assert store.latest(["r1"]) == first
        store.close()

    def test_history_cleanup_is_bounded(self, tmp_path: Path) -> None:
        store = TopologyHistoryStore(tmp_path / "nfweb.sqlite", retention_minutes=180)
        now = datetime.now(timezone.utc)
    ...
    def test_logs_are_scope_aware_and_bounded_to_300(self, tmp_path: Path) -> None:
        store = TopologyHistoryStore(tmp_path / "nfweb.sqlite", retention_minutes=180)
