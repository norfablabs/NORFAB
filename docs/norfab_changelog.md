...
2. NetBox VLAN synchronization now silently prefers descriptive live VLAN names over autogenerated `VLAN<VID>` names instead of reporting the discarded autogenerated name as a conflict.
3. NetBox VLAN synchronization now ignores empty live descriptions when another device reports a non-empty description for the same VLAN.
