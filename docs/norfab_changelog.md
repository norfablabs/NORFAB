...
1. Moved VLAN creation, attributes, tagged and untagged membership, and VLAN-derived interface mode from `sync_device_interfaces` to `sync_vlans`. The VLAN task uses the shared `interface_map` rules and returns separate `vlans` and `interfaces` diffs.
2. VLAN membership updates are additive: empty live lists preserve NetBox assignments, except that native VLANs can be replaced. The first matching `vlan_map` rule applies to a VLAN and all its memberships. Interface mapping uses substring containment; if multiple names map to one interface, only the first selected name from a resolved VLAN is processed. Q-in-Q service VLAN assignments are not supported.
3. Invalid memberships and missing VLAN groups are reported without stopping unrelated VLAN synchronization. Device scopes and VLAN groups are fetched in bulk, as are VLAN creations and attribute updates. If devices report different names for one VLAN, the first descriptive name takes precedence over `VLAN<VID>`.
4. Removed `vlan_group`, `vlan_map`, `require_vlan_group`, and `ignore_vlans` from `sync_device_interfaces` and its NFCLI command. Brief `get_interfaces` output no longer includes tagged or untagged VLAN fields.
5. `sync_all` now creates interfaces before assigning VRFs or VLANs, and stops before later stages if VLAN synchronization fails. Per-device VLAN results contain only that device's interface actions. `check_device_sync` now includes VRF object and interface-assignment drift.
6. Moved interface VRF assignment to `sync_vrfs`, using the TTP getter's normalized address-family and interface data. Results separate `vrfs`, `route_targets`, and per-device `interfaces` diffs. Matching assignments are reported as in sync; changed assignments are updated after missing VRFs are created. Interface, VLAN, and VRF synchronization share the same `interface_map` rules.
7. Missing route targets appear in VRF dry-run and approval plans; created targets appear in live results. VRF route-target IDs are resolved and cached after the diff. Duplicate same-name VRFs still resolve to the lowest ID, with a warning event and log message.
8. Added additive IPv4/IPv6 import and export BGP routing-policy associations through configurable multi-object VRF custom fields. Missing policies appear in plans, are created before VRF writes, and appear in live results. Policy processing is skipped when the BGP plugin is absent; other VRF synchronization continues.
9. Task-local caches reduce NetBox lookups. Interface synchronization reuses IDs from initial reads and bulk creations, reports ignored type-only differences as in sync, and gives dry-run precedence over approval. VRF synchronization caches selected VRFs.
10. Flattened VRF, route-target, and routing-policy diff and result sections by removing their `global` wrapper. VRF sync now emits fewer progress events and uses matching text for paired events and logs.
