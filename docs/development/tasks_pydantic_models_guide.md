...
This is a mandatory style requirement: do not place task-specific input, output, or payload models in a centralized `*_models.py` file. A centralized model module is only for models reused by more than one task module, such as common worker arguments, shared enums, or payload models consumed by multiple tasks.
...
Centralized model modules must stay small and shared-only. Before adding a model to one, confirm at least two task modules import it. If only one task uses the model, move it next to that task even if the module becomes longer.
...
7. Keep task-specific models in the task module; use centralized model files only for models reused across multiple task modules.
...
- Do not put one-task-only models in centralized model files; colocate them with the task.
